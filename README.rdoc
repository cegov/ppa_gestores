...
tirar fora barra lateral com notícias
usar boxes como do site brasil.gov.br replicando as seções do site
limitar (max_width) o conteúdo a +- 960px ou o tamanho do site do ppa
fazer galeria de vídeos (usar blueimp)
tirar obrigatoriedade de se colocar uma data de início e término dos cursos
tirar obrigatoriedade de se colocar uma data de início e término das notícias/eventos

criar campo imagem em notícia
criar image picker em notícia
criar campo default false para notícia em destaque

OK criar página de acessibilidade no servidor de testes
fazer upload da página de acessibilidade

OK BUG botão previsualizar não funciona (courses/3)

OK tratar campos null e blank nas views show de newsarticles
tratar campos null e blank nas views show de courses

usar jquery para esconder as respostas para as perguntas e mostrar on click

arrumar a lista de notícias, formatação
...
fazer a relação dos recursos dos plugins e a class Refinery_User vide:
...
verificar possível problema com backup de imagens
